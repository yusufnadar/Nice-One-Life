...
class DataService{
  ...
  Future<List<PackageModel>> getShowMyPackages() async{
    var liste = <PackageModel>[];
    var snapshot1;
    var snapshot = await _firestore.collection('myPackages').doc(userId).collection('myPackages').get();
    for(var item in snapshot.docs){
      snapshot1 = await _firestore.collection('packages').where('id',isEqualTo: item.data()['id']).get();
    }
